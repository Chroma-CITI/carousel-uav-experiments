    ...
    public:
        ...
        SimpleControl() : rclcpp::Node("SimpleControl"){
            ...
            auto cmdloop_callback = [this]() -> void {
                ...
                // if(this->nav_state == px4_msgs::msg::VehicleStatus::NAVIGATION_STATE_OFFBOARD){
                RCLCPP_INFO(this->get_logger(), "Sending trajectory setpoint to UAV %i: at velocity %f, %f, %f", this->robot_id, cmd[0], cmd[1], cmd[2]);

                this->publish_trajectory_setpoint(transform_vector_quat(cmd, this->q), NAN);
                // }
